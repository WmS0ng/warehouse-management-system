...
/*Table structure for table `check_batch` */

DROP TABLE IF EXISTS `check_batch`;
...
) ENGINE=InnoDB AUTO_INCREMENT=29 DEFAULT CHARSET=utf8mb3 COMMENT='盘点批次表';

/*Data for the table `check_batch` */

insert  into `check_batch`(`batch_id`,`batch_num`,`store_id`,`create_time`,`create_user`) values (23,'201712121120',1,'2017-12-12 11:20:30','admin'),(24,'201712121123',1,'2017-12-12 11:23:25','admin'),(25,'201712121124',1,'2017-12-12 11:24:01','admin'),(26,'201712121823',1,'2017-12-12 18:23:36','admin'),(27,'201712121841',1,'2017-12-12 18:41:47','admin'),(28,'201712141557',1,'2017-12-14 15:57:36','admin');
...
CREATE TABLE `check_record` (
  ...
  `check_state` char(1) DEFAULT '0' COMMENT '1 盘点完成  0 未盘点',
  PRIMARY KEY (`record_id`)
) ENGINE=InnoDB AUTO_INCREMENT=25 DEFAULT CHARSET=utf8mb3 COMMENT='盘点记录';

/*Data for the table `check_record` */

insert  into `check_record`(`record_id`,`product_id`,`store_id`,`batch_id`,`check_state`) values (19,1,1,23,'1'),(20,2,1,24,'1'),(21,2,1,25,'1'),(22,3,1,26,'1'),(23,4,1,27,'1'),(24,13,1,28,'1');
...
CREATE TABLE `message` (
  ...
  `create_time` datetime DEFAULT NULL,
  PRIMARY KEY (`msg_id`)
) ENGINE=InnoDB AUTO_INCREMENT=54 DEFAULT CHARSET=utf8mb3 COMMENT='站内信';

/*Data for the table `message` */

insert  into `message`(`msg_id`,`msg_title`,`msg_context`,`create_time`) values (34,'这是admin发送给root的测试信息1','这是admin发送给root的测试信息1','2017-12-09 23:46:20'),(35,'这是admin发送给root的测试信息2','这是admin发送给root的测试信息2','2017-12-09 23:46:33'),(36,'这是admin发送给root的测试信息3','这是admin发送给root的测试信息3','2017-12-09 23:46:47'),(37,'这是admin发送给root的测试信息4','这是admin发送给root的测试信息4','2017-12-09 23:47:00'),(38,'这是admin发送给root的测试信息5','这是admin发送给root的测试信息5','2017-12-09 23:47:12'),(39,'这是admin发送给root的测试信息6','这是admin发送给root的测试信息6','2017-12-09 23:47:26'),(40,'测试7','这是admiin发送给root的测试信息7','2017-12-10 00:09:31'),(41,'这是root发送给admin的测试信息1','这是root发送给admin的测试信息1','2017-12-10 00:20:00'),(42,'这是root发送给admin的测试信息2','这是root发送给admin的测试信息2，，，1.大数据：一台传统的物理计算机不能处理的数据就称为大数据，大数据还包含：多样化，价值化。（海量的数据） 2.在思维方式方面，大数据完全颠覆了传统的思维方式：，1.大数据：一台传统的物理计算机不能处理的数据就称为大数据，大数据还包含：多样化，价值化。（海量的数据） 2.在思维方式方面，大数据完全颠覆了传统的思维方式：','2017-12-10 16:28:30'),(43,'测试','1.Hadoop:开源的分布式大数据处理架构（是一个能够对大量数据进行分布式处理的软件框架） 2.分布式：n台计算机同时运算（对服务器的硬件配置要求较低） 3.Hadoop：  高可靠性 			高效性 			高可扩展性 			高容错性 			成本低 			运行在Linux平台上 			支持多种编程语言 4.Hadoop核心版本：2.0	2.0版本多增加了yarn系统 5.Hadoop的核心：HDFS:分布式数据存储系统（将得到的大量数据存储在里面） 				yarn：分布式资源调度系统（因为1.0版本中没有yarn，读取的时候是直接从mapreduce中读取， 					  大量的时间都浪费在数据的查找上面，所以yarn主要解决了这个问题， 					  相当于一个缓冲区，节省了时间） 				mapreduce:分布式数据读取系统（将存在hdfs中的大量数据读取出来，当然是有价值的读取） 				flume:日志收集系统 				spark：内存计算系统（主要是针对内存进行的操作）   最快的速度精确的结果  海量数据的分布式存储：HDFS 海量数据的分布式处理：mapreduce           flume:日志收集 价值挖掘，分析：R   										云计算  云计算的概念： 	云计算实现了通过网络提供可伸缩的，廉价的分布式计算能力，用户只需要在具备网络接入条件的地     方，就可以随时随地的获得所需要的IT（数据）资源。  云计算的关键技术 	虚拟化，分布式存储，分布式计算，多租户。   Iaas 	将基础设施（计算资源和存储）作为服务出租 Paas 	类似于Iaas，但是它包括操作系统和围绕特定应用的必须的服务。 Sass 	从一个集中的系统部署软件，使之在一台本地计算机上（或从云中远程的）运行的一个模型，由于是	计量服务，Saas允许出租一个应用的程序，并计时收费。','2017-12-10 16:34:28'),(44,'admin---5','测试','2017-12-10 21:38:25'),(45,'测试3---5','这是用户3发给用户5的测试信息1','2017-12-11 15:00:05'),(46,'测试3----5','这是用户3发给用户5的测试信息2','2017-12-11 15:00:29'),(47,'测试1---5','这是用户1发送给用户5的测试信息1','2017-12-11 15:02:49'),(48,'测试1----5','这是用户1发送给用户5的测试信息2','2017-12-11 15:03:08'),(49,'测试5----3','这是用户5发送给用户3的测试信息1','2017-12-11 15:32:29'),(50,NULL,NULL,'2017-12-12 18:34:00'),(51,NULL,NULL,'2017-12-12 18:34:01'),(52,'测试admin-----root----1','测试admin-----root----1','2017-12-12 18:34:41'),(53,'111111','是分不开的不仅拷贝不是的空间分布式就高不就低不vdfbgdkghjdfknvdfjkghergdkfgbdfvdfjkghdig','2017-12-14 16:25:18');
...
CREATE TABLE `store_check` (
  ...
  `product_id` int DEFAULT NULL,
  `batch_id` int DEFAULT NULL,
  `check_num` decimal(8,2) DEFAULT NULL,
  `check_time` date DEFAULT NULL,
  `check_user` varchar(20) DEFAULT NULL,
  `loss_num` decimal(8,2) DEFAULT NULL,
  `reson` varchar(200) DEFAULT '0',
  PRIMARY KEY (`check_id`)
) ENGINE=InnoDB AUTO_INCREMENT=27 DEFAULT CHARSET=utf8mb3 COMMENT='盘点表';

/*Data for the table `store_check` */

insert  into `store_check`(`check_id`,`store_id`,`record_id`,`product_id`,`batch_id`,`check_num`,`check_time`,`check_user`,`loss_num`,`reson`) values (21,1,19,1,23,'12.00','2017-12-12','r','1.00','sunaho'),(22,1,20,2,24,NULL,'2017-12-12','','1.00',''),(23,1,21,2,25,'123.00','2017-12-12','xxxxxx','1.00','xxx'),(24,1,22,3,26,'111111.00','2017-12-12','frfrfrfr','1.00','frfrfrfr'),(25,1,23,4,27,'123.00','2017-11-12','r','1.00','frfr'),(26,1,24,13,28,'12.00','2017-11-14','杜','1.00','sunhao');
...
DROP TABLE IF EXISTS `transshipment`;
...
CREATE TABLE `transshipment` (
  ...
  `audit_state` char(1) DEFAULT NULL COMMENT '0 未审核 1 已审核',
  `create_by` int DEFAULT NULL,
  `create_time` datetime DEFAULT NULL,
  PRIMARY KEY (`trans_id`)
) ENGINE=InnoDB AUTO_INCREMENT=16 DEFAULT CHARSET=utf8mb3 COMMENT='调货单';

/*Data for the table `transshipment` */

insert  into `transshipment`(`trans_id`,`source_store_id`,`source_product_id`,`target_store_id`,`target_product_id`,`trans_num`,`audit_state`,`create_by`,`create_time`) values (1,2,5,1,9,100,'1',1,'2017-12-13 14:42:01'),(2,2,5,1,10,21,'1',1,'2017-12-13 14:46:16'),(3,2,5,1,11,32,'1',1,'2017-12-13 15:22:04'),(4,2,5,1,12,11,'1',1,'2017-12-13 15:33:08'),(5,2,6,1,13,100,'1',1,'2017-12-13 15:37:03'),(6,2,6,1,14,222,'1',1,'2017-12-13 18:57:03'),(7,2,6,1,15,78,'1',1,'2017-12-13 19:17:59'),(8,2,6,1,16,100,'1',1,'2017-12-13 19:27:11'),(9,1,1,2,18,43,'1',1,'2017-12-14 15:39:04'),(10,1,21,2,24,20,'0',1,'2022-04-21 20:47:36'),(11,2,23,1,25,100,'0',1,'2022-04-21 21:01:24'),(12,2,13,1,26,20,'0',1,'2022-04-22 09:02:29'),(13,2,19,1,27,20,'0',1,'2022-04-22 09:16:57'),(14,2,13,1,28,20,'1',1,'2022-04-22 09:17:30'),(15,2,23,1,30,20,'1',32,'2022-04-24 15:55:47');
...
CREATE TABLE `user_message` (
  ...
  `state` char(1) DEFAULT '0' COMMENT '0 未读 1 已读',
  `from_user` int DEFAULT NULL,
  `to_user` int DEFAULT NULL,
  PRIMARY KEY (`user_msg_id`)
) ENGINE=InnoDB AUTO_INCREMENT=48 DEFAULT CHARSET=utf8mb3 COMMENT='用户站内信关系表';

/*Data for the table `user_message` */

insert  into `user_message`(`user_msg_id`,`msg_id`,`user_id`,`state`,`from_user`,`to_user`) values (26,34,3,'1',1,3),(27,35,3,'0',1,3),(28,36,3,'1',1,3),(29,37,3,'0',1,3),(30,38,3,'0',1,3),(31,39,3,'0',1,3),(32,40,3,'1',1,3),(33,41,1,'1',3,1),(34,42,1,'1',3,1),(35,43,3,'1',1,3),(36,44,5,'0',3,5),(37,45,5,'0',3,5),(38,46,5,'0',3,5),(39,47,5,'0',1,5),(40,48,5,'0',1,5),(41,49,3,'1',5,3),(42,50,3,NULL,1,3),(43,51,3,NULL,1,3),(44,52,3,NULL,1,3),(45,53,1,'1',3,1),(46,53,5,NULL,3,5),(47,53,6,NULL,3,6);
